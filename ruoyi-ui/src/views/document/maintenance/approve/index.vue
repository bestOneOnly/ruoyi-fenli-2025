<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="maintenanceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="处置时间" align="center" prop="disposeTime" />
      ...
      <el-table-column label="审批描述" align="center" prop="approveDesc" />
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope" v-if="scope.row.statusEnum.enumName==='DISPOSED'">
          <el-button size="mini" type="text" icon="el-icon-coordinate" @click="handleApproveButton(scope.row)">审批</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body @close="cancel" close-on-click-modal>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="审批描述" prop="name">
          <el-input v-model="form.approveDesc" placeholder="请输入审批内容" />
        </el-form-item>
      </el-form>
...
<script>
import { listMaintenanceApprove, getMaintenance, handleApprove, getMaintenanceStatus } from "@/api/document/maintenance"
...
export default {
  ...
  methods: {
    ...
    //是否显示审批按钮
    isShowHandleButton({status}){
        return this.statusList.find(item => item.value === status)?.engLabel === 'DISPOSED';
    },
    /** 查询保养工单列表 */
    getList() {
      this.loading = true
      listMaintenanceApprove(this.queryParams).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          handleApprove(this.form).then(response => {
